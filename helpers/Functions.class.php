...
class Functions {
	...
	function __construct($sys){
            ...
            //$this->saslog = $this->saslog('full');
            //$this->userlog = $this->userlog('full');
	...
	function formatCNPJ($cnpj = null) {
		$result = "CNPJ inválido";
		if(strlen($cnpj) == 14){
			$cnpjpart1 = substr($cnpj,0,2);
			$cnpjpart2 = substr($cnpj,2,3);
			$cnpjpart3 = substr($cnpj,5,3);
			$cnpjpart4 = substr($cnpj,8,4);
			$cnpjpart5 = substr($cnpj,12,2);
			$result = $cnpjpart1.".".$cnpjpart2.".".$cnpjpart3."/".$cnpjpart4."-".$cnpjpart5;
		}else if(strlen($cnpj) == 15){
			$cnpjpart1 = substr($cnpj,0,3);
			$cnpjpart2 = substr($cnpj,2,3);
			$cnpjpart3 = substr($cnpj,5,3);
			$cnpjpart4 = substr($cnpj,8,4);
			$cnpjpart5 = substr($cnpj,12,2);
	...
	function validaCNPJ($cnpj){
		...
		$cnpj = (string)$cnpj;
		$cnpj = (strlen($cnpj)==15?substr($cnpj,1,14):$cnpj);
		$cnpj_original = $cnpj;
		$primeiros_numeros_cnpj = substr( $cnpj, 0, 12 );
		
		if (!function_exists('multiplica_cnpj')){
			function multiplyCNPJ($cnpj,$posicao=5){
				$calculo = 0;
		...
		$primeiro_calculo = multiplyCNPJ($primeiros_numeros_cnpj);
		$primeiro_digito = ( $primeiro_calculo % 11 ) < 2 ? 0 :  11 - ( $primeiro_calculo % 11 );
		$primeiros_numeros_cnpj .= $primeiro_digito;
		$segundo_calculo = multiplyCNPJ($primeiros_numeros_cnpj,6);
	...
	function formatCPF($cpf = null) {
		$result = "CPF inválido";
		if(strlen($cpf) == 11){
			$cpfpart1 = substr($cpf,0,3);
			$cpfpart2 = substr($cpf,3,3);
			$cpfpart3 = substr($cpf,6,3);
			$cpfpart4 = substr($cpf,9,2);
			$result = $cpfpart1.".".$cpfpart2.".".$cpfpart3."-".$cpfpart4;
		}
		return $result;
	}
	...
	public function onlineCheck(){
            if(strpos($this->system->params['sysco']['domain'],$this->system->params['sysco']['baseurl']) !== false){
                return true;
            }else{
                return false;
            }
